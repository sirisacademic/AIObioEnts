...
class LRSchedule_LINEAR(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(
        self,
        init_lr=2e-5,#BioLinkBERT-large: 1e-5
        init_warmup_lr=0.0,
        final_lr=5e-6,#BioLinkBERT-large: 2e-6
        warmup_steps=0,
        decay_steps=400,
    ):
        super().__init__()
    ...
    def __call__(self, step):
        ...
        decay_lr = tf.math.maximum(
            tf.cast(self.final_lr, tf.float64),
            self.init_lr - (step - self.warmup_steps)/self.decay_steps*(self.init_lr - self.final_lr)
        )

        return tf.math.minimum(tf.cast(warmup_lr, tf.float64),decay_lr)

    def get_config(self):
        config = {
            'init_lr': self.init_lr,
            'init_warmup_lr': self.init_warmup_lr,
...
class HUGFACE_NER(): #huggingface transformers
    ...
    def build_encoder(self):
        ...
        x = plm_model.bert(x1_in, token_type_ids=x2_in, attention_mask=x3_in)[0]
