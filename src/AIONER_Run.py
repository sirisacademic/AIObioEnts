...
from processing_data import ml_intext_fn,out_BIO_BERT_crf_fn
...
def ML_Tag(text,ml_model,entity_type='ALL'):

    conll_in=ssplit_token(text, entity_type, max_len=ml_model.maxlen)
    
    ml_tsv=ml_tagging(conll_in,ml_model)
    ...
    with open(infile, 'r',encoding='utf-8') as fin:
        with open(outfile,'w', encoding='utf8') as fout:
            ...
            for doc in all_text:
                ...
                tag_result=ML_Tag(intext,nn_model,entity_type=para_set['entity_type'])
                fout.write(lines[0]+'\n'+lines[1]+'\n')
            ...
            for document in collection.documents:
                ...
                for passage in document.passages:
                    if passage.text!='' and (not passage.text.isspace()) and passage.infons['type']!='ref': # have text and is not ref
                        ...
                        tag_result=ML_Tag(passage.text,nn_model,entity_type=para_set['entity_type'])
                        for ele in tag_result:
...
if __name__=="__main__":
    ...
    parser.add_argument('--inpath', '-i', help="input path",default='')
    parser.add_argument('--model', '-m', help="trained deep learning NER model file",default='../models/pubmedbert/pubmedbert-es-AIO.h5')
    ...
    parser.add_argument('--outpath', '-o', help="output path to save the NER tagged results",default='')
